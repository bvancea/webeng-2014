class MembershipsController < ApplicationController
  ...
  def create(user, group)
    ...
    if @membership.save
      flash[:success] = "Membership user: "+ User.find(user.id).name+ " to group: "+ Group.find(group.id)+ " successfully created!"
    else
      flash[:error] = "Membership user: "+ User.find(user.id).name+ " to group: "+ Group.find(group.id)+ " cannot be created!"
    end
  end
